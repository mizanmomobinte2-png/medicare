...
router.patch(
  ...
  h(async (req, res) => {
    const id = idOf(req);
    const { appt_date, appt_time, notes } = req.body;

    if (appt_date && !isDate(String(appt_date).slice(0, 10))) {
    ...
    const r = await pool.query(
      `
      UPDATE appointment
      SET appt_date = COALESCE($1, appt_date),
          appt_time = COALESCE($2, appt_time),
          notes     = COALESCE($3, notes)
      WHERE appt_id = $4 AND staff_id = $5
      RETURNING *
      `,
      [
        appt_date ? String(appt_date).slice(0, 10) : null,
        appt_time || null,
        notes ?? null,
        id,
        req.staff.staff_id,
...
router.get(
  ...
  h(async (req, res) => {
    const r = await pool.query(
      ...
      WHERE lt.status IN ('requested', 'scheduled')
...
router.post(
  ...
  h(async (req, res) => {
    ...
    const r = await pool.query(
      ...
      WHERE test_id = $2 AND staff_id IS NULL AND status = 'requested'
      RETURNING *
      `,
      [req.staff.staff_id, id]
    );
    ...
    if (r.rows.length === 0) {
      throw new HttpError(409, "Another staff already took this lab test");
    }
...
router.get(
  ...
  h(async (req, res) => {
    const r = await pool.query(
      ...
      WHERE s.status IN ('recommended', 'scheduled')
...
router.post(
  ...
  h(async (req, res) => {
    ...
    const r = await pool.query(
      ...
      WHERE surgery_id = $2 AND staff_id IS NULL AND status = 'recommended'
      RETURNING *
      `,
      [req.staff.staff_id, id]
    );
    ...
    if (r.rows.length === 0) {
      throw new HttpError(409, "Another staff already took this surgery");
    }
...
router.patch(
  ...
  h(async (req, res) => {
    ...
    const r = await pool.query(
      ...
      WHERE surgery_id = $2
        AND staff_id = $3
        AND status IN ('recommended', 'scheduled')
      RETURNING *
      `,
...
const BILL_JOINS = `
  ...
  LEFT JOIN patient pt
    ON pt.patient_id = COALESCE(ad.patient_id, ap.patient_id, sg.patient_id, lt.patient_id)
...
const SOURCE_COLS = `
  CASE
    ...
    ELSE 'other'
  END AS source_type,
  COALESCE(b.appt_id, b.adm_id, bs.surgery_id, bl.test_id) AS source_id
`;
